...
<body>
	<footer id="footer" style="margin: 10% auto;width:50%;height:30%;">
		<section>
		<h2>화장품 정보 ${ product.p_no > 0 ? "수정" : "등록" }</h2>
		<form method="POST" modelAttribute="ProductVO" id="frm">
			...
			<div class="field">
				<label for="name">NAME</label> <input type="text" name="p_name" value="${product.p_name}"/>
			</div>
			<div class="field">
				<label for="img">IMG</label> <input type="text" name="p_img" value="${product.p_img}"/>
			</div>
			<div class="field">
				<label for="detail">Detail</label>
				<textarea name="p_detail" rows="3" value="${product.p_detail}"></textarea>
			</div>
			<div class="field">
				<label for="ingredient">INGREDIENT</label>
				<textarea name="p_ingredient" rows="3" value="${product.p_ingredient}"></textarea>
			</div>
	
			<div class="field">
				<label for="grade">GRADE</label> <input type="text" name="p_grade" value="${product.p_grade}"/>
			</div>
		</form>
		<ul class="actions">
